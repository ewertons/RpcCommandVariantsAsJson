...
AZ_INLINE az_result vehicle03_unlock_begin(
    ...
    long int request_timestamp,
    const uint8_t* requested_from, int32_t requested_from_size)
{
  az_span payload = AZ_SPAN_FROM_STR(
              "{\"RequestTimestamp\":1691530585198,\"RequestedFrom\":\"mobile-app\"}");

  command_data->correlation_id = az_span_create((uint8_t*)correlation_id, correlation_id_size);
  command_data->content_type = vehicle03_unlock_payload_content_type;
  command_data->request_payload = payload;
  command_data->command_name = vehicle03_unlock_command_name;
